...
pub fn print_components(
    query: Query<Entity, With<UiTextureAtlasImage>>,
...
pub fn debug_plugin(app: &mut App) {
    app.add_plugins(RapierDebugRenderPlugin::default())
        .add_systems(Update, save_debug)
        .insert_resource(ExampleUiTnuaActive(true))
        .insert_resource(Debug {
            enabled: false,
            skip_start_screen: false,
        })
        .add_systems(Startup, set_debug)
        .add_systems(OnEnter(GameState::Start), skip_start_screen)
        .add_plugins(EguiPlugin)
        .add_systems(Update, toggle_debug)
        .add_systems(Update, debug_rendering)
        .add_systems(Update, ui_system.run_if(run_debug))
        .add_systems(Update, plot_source_rolling_update.run_if(run_debug))
        .add_systems(Update, track_player.run_if(run_debug));
    // app.add_systems(Update, print_components);
}
        ...
        )) = query.get_single_mut()
        {
            ui.horizontal(|ui| {
                ui.vertical(|ui| {
                    ...
                    egui::CollapsingHeader::new("Player Movement")
                        .default_open(false)
                        .show(ui, |ui| {
                            ui.vertical(|ui| {
                                ui.add(
                                    egui::Slider::new(
                                        &mut platformer_config.float_height,
                                        0.0..=10.0,
                                    )
                                    .text("Float At"),
                                );
                                ...
                                ui.add(
                                    egui::Slider::new(
                                        &mut platformer_config.jump_input_buffer_time,
                                        0.0..=1.0,
                                    )
                                ...
                                let free_fall_options: [(bool, &str, fn() -> TnuaFreeFallBehavior);
                                    3] = [
                                    (
                                        matches!(
                                ...
                                {
                                    ui.add(
                                        egui::Slider::new(extra_gravity, 0.0..=100.0)
                                            .text("Extra Gravity"),
                                    );
                                }
